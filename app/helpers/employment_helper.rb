module EmploymentHelper
  def employment_links(check, partner: false)
    if check.controlled?
      {
        t("estimate_flow.income.guidance.text") => document_link(:lc_guidance_controlled, :self_employed),
      }
    else
      key = "estimate_flow.#{'partner_' if partner}income.police_guidance.text"
      {
        t("estimate_flow.income.guidance.text") => document_link(:lc_guidance_certificated, :self_employed),
        t(key) => document_link(:lc_guidance_certificated, :police),
      }
    end
  end
end
