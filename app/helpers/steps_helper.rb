module StepsHelper
  ...
  TAIL_STEPS = %i[assets check_answers results].freeze
  ...
  ALL_POSSIBLE_STEPS = (%i[applicant] + STEPS_WITH_PROPERTY + ALL_VEHICLE_STEPS + TAIL_STEPS).freeze
...
private
  ...
  def steps_list_for(estimate)
    ...
    ([%i[applicant]] + non_tail_steps.map { |step| [step] } + [vehicle_steps] + TAIL_STEPS.map { |step| [step] }).freeze
  end
