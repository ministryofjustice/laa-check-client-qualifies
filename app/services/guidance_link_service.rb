class GuidanceLinkService
  class << self
    def call(document:, sub_section: nil, page_number_only: false)
      if page_number_only
        mapping[:mtr_phase_1_links].dig(document, :sections).fetch(sub_section)
      elsif sub_section
        "#{mapping[:mtr_phase_1_links].dig(document, :page_url)}#page=#{mapping[:mtr_phase_1_links].dig(document, :sections).fetch(sub_section)}"
      else
        mapping[:mtr_phase_1_links].dig(document, :page_url)
      end
    end
