...
RSpec.describe "Controlled other income", type: :feature do
  ...
  context "when hitting the API", :integration do
    context "with certificated" do
      include_context "with certificated other income"

      it "produces the correct results table for certificated work - in particular the other income frequency" do
        submit_form
    ...
    context "with controlled" do
      ...
      it "produces the correct results table for controlled work - in particular the other income frequency" do
        submit_form
        expect(page).to have_content("Financial help from friends and family\n£866.67")
      end
    end
