...
RSpec.describe "Certificated check without partner", type: :feature do
  ...
  context "when interacting with the CFE API", :end2end do
    ...
    it "shows appropriate content on the results screen" do
      ...
      expect(page).to have_content "We estimate they will have to pay towards the costs of their case:\n£32.58 per month from their disposable income - these contributions will continue for the duration of the case, however long it lasts£0.00 lump sum payment from their disposable capital - any capital contribution will not exceed the likely costs of their case and they might pay less than this amount"
      ...
      expect(page).to have_content "Total monthly income£883.66"
      expect(page).to have_content "Employment expenses\nA fixed allowance if your client gets a salary or wage\n£45.00"
      expect(page).to have_content "Housing costs minus any Housing Benefit payments your client gets\n£100.00"
      expect(page).to have_content "Dependants allowance\nAn allowance of £361.70 applied for each dependant, minus any income they receive\n£334.57"
      expect(page).to have_content "Total monthly outgoings£479.57"
      ...
      expect(page).to have_content "Total assessed disposable capital£701.00"
