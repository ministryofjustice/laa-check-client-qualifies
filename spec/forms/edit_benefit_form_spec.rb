...
RSpec.describe "benefits/new", type: :feature do
  ...
  it "shows an error message if values are deleted" do
    fill_in "benefit_model[benefit_amount]", with: ""
    click_on "Save and continue"
    expect(page).to have_content "Enter the amount"
  end

  it "saves what I enter to the session" do
    fill_in "benefit_model[benefit_type]", with: "B"
    fill_in "benefit_model[benefit_amount]", with: "2"
    choose "Every 2 weeks"
    click_on "Save and continue"
    expect(session_contents.dig("benefits", 0, "benefit_type")).to eq "B"
    expect(session_contents.dig("benefits", 0, "benefit_amount")).to eq 2
    expect(session_contents.dig("benefits", 0, "benefit_frequency")).to eq "every_two_weeks"
    expect(session_contents.dig("benefits", 0, "id")).to eq benefit_id
  end
end
