...
RSpec.describe "income", type: :feature do
  let(:assessment_code) { :assessment_code }
  let(:session_data) { { "level_of_help" => level_of_help } }
  let(:level_of_help) { "controlled" }
  let(:annual_option) { "Total in the last year" }

  before do
    set_session(assessment_code, session_data)
    visit "estimates/#{assessment_code}/build_estimates/income"
  end
  ...
  it "saves what I enter to the session" do
    ...
    click_on "Save and continue"
    expect(session_contents.dig("incomes", 0, "gross_income")).to eq 123
    expect(session_contents.dig("incomes", 0, "income_tax")).to eq 22
    expect(session_contents.dig("incomes", 0, "national_insurance")).to eq 11
    expect(session_contents.dig("incomes", 0, "income_type")).to eq "employment"
    expect(session_contents.dig("incomes", 0, "income_frequency")).to eq "every_week"
  end
  ...
  context "when this is a certificated check" do
    ...
    it "saves what I enter to the session" do
      ...
      fill_in "income_model[items][1][national_insurance]", with: "11"
      click_on "Save and continue"
      expect(session_contents.dig("incomes", 0, "income_frequency")).to eq "year"
    end
  end
end
