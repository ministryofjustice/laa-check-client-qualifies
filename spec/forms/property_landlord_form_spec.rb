...
RSpec.describe "property_landlord", :calls_cfe_early_returns_not_ineligible, type: :feature do
  ...
  let(:title_for_cannot_use_service) { I18n.t("question_flow.property.cannot_use_service.title") }
  ...
  context "when client is means-tested and reaches the `cannot-use-service` page" do
    before do
      start_assessment
      fill_in_forms_until(:applicant)
      if partner
    ...
    it "shows correct content on `cannot-use-service` page", :shared_ownership do
      expect(page).to have_content title_for_cannot_use_service
    end
  end
  ...
  context "when client is non-means-tested and reaches the `cannot-use-service` page" do
    before do
      start_assessment
      fill_in_client_age_screen(choice: "Under 18")
      fill_in_level_of_help_screen(choice: "Civil controlled work or family mediation")
  ...
  context "when client is passported and reaches the `cannot-use-service` page" do
    let(:passporting) { "Yes" }
