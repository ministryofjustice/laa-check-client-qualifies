...
RSpec.describe "shared_ownership_housing_costs", :calls_cfe_early_returns_not_ineligible, :shared_ownership, type: :feature do
  ...
  it "performs validations if I enter invalid values" do
    ...
    choose "Every 2 weeks", name: "shared_ownership_housing_costs_form[rent_frequency]"
  ...
  it "stores my shared ownership housing payments responses in the session" do
    ...
    choose "Every 4 weeks", name: "shared_ownership_housing_costs_form[mortgage_frequency]"
    click_on "Save and continue"

    expect(session_contents["rent"]).to eq 200
    expect(session_contents["rent_frequency"]).to eq "every_two_weeks"
    ...
    expect(session_contents["mortgage_frequency"]).to eq "every_four_weeks"
  end
end
