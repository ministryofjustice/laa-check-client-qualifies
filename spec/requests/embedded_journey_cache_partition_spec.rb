...
RSpec.describe "embedded journey cache partitioning", :embedded_only, ccq_mode: :embedded, type: :request do
  ...
  before do
    ...
    allow(host_service_client).to receive(:load) do |**kwargs|
      cookies = kwargs.fetch(:cookies)
  ...
  it "keeps two users' journeys separate for the same resource" do
    get "/cases/#{resource_id}/eligibility", headers: { "Cookie" => "service.sid=user-a" }
    expect(response).to be_redirect

    get "/cases/#{resource_id}/eligibility", headers: { "Cookie" => "service.sid=user-b" }
    expect(response).to be_redirect
  ...
  it "deletes only the completing user's journey" do
    ...
    post "/cases/#{resource_id}/eligibility/complete", headers: { "Cookie" => "service.sid=user-a" }
