...
RSpec.describe "results/show.html.slim" do
  ...
  describe "Client financial content" do
    ...
    it "shows outgoings content" do
      ...
      expect(page_text).to include "Dependants allowanceAn allowance of £361.70 applied for each dependant, minus any income they receive£13.00"
    ...
    context "when shared ownership flag is on", :shared_ownership do
      let(:api_response) do
        FactoryBot.build(
          :api_result,
          eligible: "eligible",
      ...
      it "shows correct outgoings content" do
        expect(page_text).to include "Mortgage paymentsNot applicable"
        expect(page_text).to include "Housing payments£500.00"
        expect(page_text).to include "Housing Benefit£20.00"
      end
    end
